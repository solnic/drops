defmodule Drops.Operations.Extensions.Telemetry do
  ...
  def emit_step_stop(operation_module, _step, context, result, config) do
    ...
    # Use updated context from step result if available, otherwise fall back to original context
    updated_context = extract_context_from_result(result, context)
    ...
    case result do
      {:ok, _} ->
        :telemetry.execute(
          ...
          %{duration: duration},
          %{operation: operation_module, step: actual_step, context: updated_context}
        )
  ...
  # Extract context from step result, falling back to original context if not available
  defp extract_context_from_result({:ok, new_context}, original_context)
       when is_map(new_context) do
    # Only use the new context if it looks like a context map (has common context keys)
    # Otherwise, it might be an Ecto struct or other return value
    if has_context_structure?(new_context) do
      new_context
    else
      original_context
    end
  ...
  defp extract_context_from_result(
         {:error, %Ecto.Changeset{} = changeset},
         original_context
       ) do
    # For Ecto changeset errors, update the context with the validated changeset
    # so debug logs show the correct validation state
    if Code.ensure_loaded?(Ecto.Changeset) do
      Map.put(original_context, :changeset, changeset)
    else
      original_context
    end
  end

  defp extract_context_from_result(_result, original_context) do
    original_context
  end

  # Check if a map looks like a context map rather than an Ecto struct or other data
  defp has_context_structure?(map) when is_map(map) do
    # Context maps typically have keys like :params, :changeset, etc.
    # Ecto structs have __struct__ and __meta__ keys
    context_keys = [:params, :changeset, :schema, :data]
    ecto_keys = [:__struct__, :__meta__]

    has_context_keys = Enum.any?(context_keys, &Map.has_key?(map, &1))
    has_ecto_keys = Enum.any?(ecto_keys, &Map.has_key?(map, &1))

    # It's a context if it has context keys and doesn't have Ecto struct keys
    has_context_keys and not has_ecto_keys
  end
end
